                             ...
                             int *ierr)
{
    ...
    *igotids = 0;
    ...
    for (i = 0; i < nattrib; i++)
    {
        ...
        {
            if (debug)
                printf("DEBUG: unknown attribute %s \n", name);
        }

        H5T_class_t type_class = H5Tget_class(type_id);

        if (type_class == H5T_INTEGER || type_class == H5T_FLOAT || type_class == H5T_NATIVE_DOUBLE) // Only read numeric types
        {
        ...
        {
            if (debug)
                printf("DEBUG: Attribute %s is not numeric and will be skipped.\n", name);
        }
    ...
    if (debug)
        printf("DEBUG: number of datasets = %i \n", (int)ndatasets);
    ...
    *ncol = *ncol + *ndim;
    ...
    set_blocklabel_gadget(&j, ndimV, "Velocities");
    *ncol = *ncol + *ndimV;
    if (*ndimV > 0)
    {
        j++;
    ...
    {
        set_blocklabel_gadget(&j, &rank, "Masses");
        *ncol = *ncol + rank;
        if (rank > 0)
            j++;
    }
    ...
    if (*ndim == 0 || *ndimV == 0)
    {
        printf("ERROR: got ndim = %i, ndimV = %i\n", *ndim, *ndimV);
        *ierr = 3;
        return;
    }
    ...
    for (i = 0; i < (int)ndatasets; i++)
    {
        ...
        {
            if (debug)
                printf("DEBUG: skipping %s as it is not a dataset\n", name);
        }
    }
    ...
    if (debug)
        printf("DEBUG: finished header read \n");
}
                            ...
                            int *iblock)
{
    ...
    if (!checkfordataset(group_id, datasetname))
    {
        ierr = 1;
        return ierr;
    }
    ...
    else
    {
        ...
        for (k = 1; k <= rank; k++)
        {
            *j = *j + 1;
            ...
            if (!H5Sselect_valid(dataspace_id))
            {
                printf("ERROR selecting hyperslab \n");
                ierr = 5;
            }
            ...
            {
                if (debug)
                    printf("DEBUG: skipping %s in COLUMN %i \n", datasetname, *j);
            }
        }
    }
    free(temp);
